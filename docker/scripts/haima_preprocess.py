...
使用示例:
  ...
  python {os.path.basename(__file__)} -i input.csv -o config.yaml -d /path/to/raw_data --dry-run
        ...
        parser.add_argument('--coscli-log-dir', 
                          default='/haplox/users/chenjh/haima/snakemake/logs/coscli_download/',
                          help='coscli日志文件目录 (默认: /haplox/users/chenjh/haima/snakemake/logs/coscli_download/)')
    ...
    def download_file(self, cos_path, local_path, dry_run=False, force=False, coscli_log_file=None):
        """下载单个文件从COS到本地
        ...
        Args:
            ...
            dry_run (bool): 是否模拟运行
            force (bool): 是否强制重新下载
            coscli_log_file (str): coscli日志文件路径
        ...
        try:
            ...
            env = os.environ.copy()
            if coscli_log_file:
                env['COSCLI_LOG_PATH'] = coscli_log_file
    ...
    def process_samples(self, args):
        ...
        # 设置coscli日志文件路径
        coscli_log_dir = args.coscli_log_dir
        os.makedirs(coscli_log_dir, exist_ok=True)
        coscli_log_file = os.path.join(coscli_log_dir, f"coscli_download_{time.strftime('%Y%m%d_%H%M%S')}.log")
        print(f"📝 coscli日志文件: {coscli_log_file}")
        ...
        with tqdm(total=len(df), desc="样本处理进度", unit="样本") as pbar:
            for index, row in df.iterrows():
                ...
                r1_result = self.download_file(cos_r1, local_r1_path, 
                                             args.dry_run, args.force, coscli_log_file)
                ...
                config_data[lib_number] = {
                    ...
                    'Coscli_Log_File': coscli_log_file  # 添加日志文件路径信息
                }
    ...
    def run(self):
        ...
        print(f"  coscli日志目录: {args.coscli_log_dir}")
